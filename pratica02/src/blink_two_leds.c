...
#define CM_PER_GPIO2_CLKCTRL_MODULEMODE_ENABLE					0x02
...
#define LED_EXT_01												(1<<28) //gpio1_28
#define LED_EXT_02												(1<<1)  //gpio2_1
...
void ledInit( ){
	...
	HWREG(SOC_CONTROL_REGS+CM_conf_gpmc_clk) |= GPIO_PIN_MODE;
	 ...
	 *  set pin direction 
	 *-----------------------------------------------------------------------------*/
	HWREG(SOC_GPIO_1_REGS+GPIO_OE) &= ~(LED_EXT_01) & ~(LED_USR0) & ~(LED_USR1) & ~(LED_USR2) & ~(LED_USR3);
 ...
 * ===  FUNCTION  ======================================================================
 *         Name:  ledToggle
 *  Description: inverts the current value in the pin responsible for led's charging
 * =====================================================================================
 */
void ledToggle( ){
		...
		if(flagBlink){
			HWREG(SOC_GPIO_1_REGS+GPIO_SETDATAOUT) = (LED_EXT_01) | (LED_USR0) | (LED_USR2);
			HWREG(SOC_GPIO_1_REGS+GPIO_CLEARDATAOUT) = (LED_USR1) | (LED_USR3);
			HWREG(SOC_GPIO_2_REGS+GPIO_CLEARDATAOUT) = (LED_EXT_02);
		}else{
			HWREG(SOC_GPIO_1_REGS+GPIO_CLEARDATAOUT) = (LED_EXT_01) | (LED_USR0) | (LED_USR2);
